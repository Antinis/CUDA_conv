                                   ...
                                   uint8_t *__restrict__ b) 
{
    ...
    __shared__ uint8_t shared_a[kernel][kernel][in_channel];
    for(int i=0; i<kernel; i++)
        for(int j=0; j<kernel; j++)
        {
            int kx=x-kernel/2+i;
            ...
            if(!(kx<0 || kx>=size || ky<0 || ky>=size))
                shared_a[i][j][out_channel_id]=a(batch_id, kx, ky, out_channel_id);
            else
                shared_a[i][j][out_channel_id]=0;
        }
    ...
    for(int in_channel_id=0; in_channel_id<in_channel; in_channel_id++)
        for(int k=0; k<kernel; k++)
            for(int l=0; l<kernel; l++)
                conv+=shared_a[k][l][in_channel_id]*w(k, l, in_channel_id, out_channel_id);
