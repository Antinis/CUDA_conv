...
)
{
    ...
    __shared__ uint8_t Ws[2][block_size_k][block_size_n]; //没有转置过

    uint8_t accum[thread_size_y][thread_size_x]={0};

    //线程用寄存器
    uint8_t A_reg[2][thread_size_y];  //长度8
    uint8_t W_reg[2][thread_size_x];

    //每个线程载入shared memory的暂存寄存器，容量是shared memory的容量的1/线程数倍
    uint8_t load_reg_a[4];
    uint8_t load_reg_w[4];

    //载入使用数据类型：float（4倍向量）
    const int thread_used_to_load_shared_memory_per_row_a=block_size_k/4; //A块的每行用几个线程去读取，2
    const int thread_used_to_load_shared_memory_per_row_m=block_size_n/4; //M块的每行用几个线程去读取，32
    
    //单位是uint8_t格子
    const int A_row_id_this_thread=thread_id/thread_used_to_load_shared_memory_per_row_a;
    const int A_col_id_this_thread=thread_id%thread_used_to_load_shared_memory_per_row_a*4;
    const int W_row_id_this_thread=thread_id/thread_used_to_load_shared_memory_per_row_m;
    const int W_col_id_this_thread=thread_id%thread_used_to_load_shared_memory_per_row_m*4;
    ...
    do
    {
        now_k_place+=block_size_k;
        //读入shared mem
        if(now_k_place<k)
        {
            reinterpret_cast<float *>(&load_reg_a)[0]
            =reinterpret_cast<float *>(&a_mat[k*(block_m_id*block_size_m+A_row_id_this_thread)+now_k_place+A_col_id_this_thread])[0];
        
            reinterpret_cast<float *>(&load_reg_w)[0]
            =reinterpret_cast<float *>(&w_mat[(now_k_place+W_row_id_this_thread)*n+W_col_id_this_thread])[0];
        }

        int place_to_load_shared=place_to_write_shared^1;
        //7次小迭代，因为block_size_k=8
        #pragma unroll
        for(int sub_iteration=0; sub_iteration<block_size_k-1; sub_iteration++)
        {   
            //数据读入寄存器
            #pragma unroll
            for(int i=0; i<thread_size_y; i+=4)
                reinterpret_cast<float *>(&A_reg[(sub_iteration+1)%2][i])[0]=
                reinterpret_cast<float *>(&As[place_to_load_shared][sub_iteration+1][thread_size_y*thread_y_id+i])[0];
            #pragma unroll
            for(int i=0; i<thread_size_x; i+=4)
                reinterpret_cast<float *>(&W_reg[(sub_iteration+1)%2][i])[0]=
                reinterpret_cast<float *>(&Ws[place_to_load_shared][sub_iteration+1][thread_size_x*thread_x_id+i])[0];
            ...
            for(int i=0; i<thread_size_y; i++)
                ...
                for(int j=0; j<thread_size_x; j++)
                    accum[i][j]+=A_reg[sub_iteration%2][i]*W_reg[sub_iteration%2][j];
        }

        //做完计算后，可以认为从global到reg的搬运已经做完了，现在将数据从reg搬到shared里
        As[place_to_write_shared][A_col_id_this_thread][A_row_id_this_thread]=load_reg_a[0];
        As[place_to_write_shared][A_col_id_this_thread+1][A_row_id_this_thread]=load_reg_a[1];
        As[place_to_write_shared][A_col_id_this_thread+2][A_row_id_this_thread]=load_reg_a[2];
        As[place_to_write_shared][A_col_id_this_thread+3][A_row_id_this_thread]=load_reg_a[3];

        //读W
        reinterpret_cast<float *>(&Ws[place_to_write_shared][W_row_id_this_thread][W_col_id_this_thread])[0]
        =reinterpret_cast<float *>(&load_reg_w)[0]; 
        ...
        //最后一次小迭代，这时取进reg的数据是下一轮大迭代的数据
        //数据读入寄存器
        #pragma unroll
        for(int i=0; i<thread_size_y; i+=4)
            reinterpret_cast<float *>(&A_reg[0][i])[0]=
            reinterpret_cast<float *>(&As[place_to_load_shared^1][0][thread_size_y*thread_y_id+i])[0];
        #pragma unroll
        for(int i=0; i<thread_size_x; i+=4)
            reinterpret_cast<float *>(&W_reg[0][i])[0]=
            reinterpret_cast<float *>(&Ws[place_to_load_shared^1][0][thread_size_x*thread_x_id+i])[0];
    ...
    for(int i=0; i<thread_size_y; i++)
        ...
        for(int j=0; j<thread_size_x; j+=4)
            reinterpret_cast<float *>(&b_mat[n*(block_size_m*block_m_id+thread_size_y*thread_y_id+i)+block_size_n*block_n_id+thread_size_x*thread_x_id+j])[0]
            =reinterpret_cast<float *>(&accum[i][j])[0];
